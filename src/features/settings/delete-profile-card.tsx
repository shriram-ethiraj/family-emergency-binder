...
export function DeleteProfileCard() {
  ...
  return <Card className="border border-destructive/40"><CardHeader><span className="mb-1 grid size-10 place-items-center rounded-xl bg-destructive/10 text-destructive"><Trash2 className="size-5" /></span><CardTitle>Delete profile</CardTitle><CardDescription>Permanently remove this profile and every record stored for it.</CardDescription></CardHeader><CardContent><AlertDialog open={open} onOpenChange={close}><AlertDialogTrigger asChild><Button variant="destructive"><Trash2 />Delete profile</Button></AlertDialogTrigger><AlertDialogContent><form onSubmit={(event) => { event.preventDefault(); remove.mutate(); }}><AlertDialogHeader><AlertDialogMedia className="text-destructive"><Trash2 /></AlertDialogMedia><AlertDialogTitle>Delete {profileName}?</AlertDialogTitle><AlertDialogDescription>This permanently deletes this profile, its documents, and every revision. Other profiles in the vault are unaffected.</AlertDialogDescription></AlertDialogHeader><div className="mt-5 space-y-4"><Field><FieldLabel htmlFor="delete-profile-name">Type {profileName} to confirm</FieldLabel><Input id="delete-profile-name" value={confirmation} onChange={(event) => setConfirmation(event.target.value)} autoComplete="off" /></Field><Field><FieldLabel htmlFor="delete-profile-password">Confirm vault password</FieldLabel><Input id="delete-profile-password" type="password" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="current-password" /></Field>{error && <p className="text-sm text-destructive" role="alert">{error}</p>}</div><AlertDialogFooter className="mt-5"><AlertDialogCancel disabled={remove.isPending}>Cancel</AlertDialogCancel><Button type="submit" variant="destructive" disabled={confirmation !== profileName || !password || remove.isPending}>{remove.isPending ? <><LoaderCircle className="animate-spin" />Deleting…</> : "Delete permanently"}</Button></AlertDialogFooter></form></AlertDialogContent></AlertDialog></CardContent></Card>;
}
