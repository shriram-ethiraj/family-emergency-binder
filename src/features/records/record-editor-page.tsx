...
export function Component() {
  ...
  const requiredFields = template.data.schema.required ?? [];
  const invalidFields = showRequired ? new Set(requiredFields.filter((path) => { const value = data[path]; return value === undefined || value === ""; })) : new Set<string>();
  const submit = (values: Values) => {
    const missing = requiredFields.filter((path) => { const value = values.data[path]; return value === undefined || value === ""; });
    if (missing.length) {
      setShowRequired(true);
      window.document.getElementById(`field-${missing[0]}`)?.focus();
      return;
    }
  ...
  return <form className="space-y-6 pb-24" onSubmit={form.handleSubmit(submit)} noValidate><PageHeader eyebrow={template.data.name} title={creating ? "Create document" : "Edit document"} description={creating ? template.data.description : "Saving creates a new immutable revision; earlier versions remain available."} actions={<div className="flex gap-2"><Badge variant="secondary">{template.data.page.size}</Badge>{!creating && document.data && <Badge variant="outline">Next revision {document.data.currentRevision + 1}</Badge>}</div>} />
    ...
    <DynamicForm template={template.data} value={data} onChange={(next: JsonObject) => form.setValue("data", next, { shouldDirty: true })} disabled={save.isPending} invalidFields={invalidFields} />
