...
- The supported deployment is one trusted, extracted application folder opened in a current desktop browser by a single user. The HTML and fixed `assets/` are trusted and must remain together. Chromium provides direct-save mode; Firefox and Safari use an explicit encrypted-download compatibility mode because they lack equivalent user-visible writable handles.
- The application HTML and fixed assets, browser, extensions, operating system, and privileged host users are trusted while a vault is unlocked. Session-selected template JSON is untrusted input.
...
- The production package has no API calls or remote assets. Its meta Content Security Policy disables networking, hash-pins the inline application, permits trusted same-folder PDF scripts and fonts, and allows inline CSS because browser UI libraries require runtime positioning styles.
- Template folders are selected explicitly before vault access. JSON files are read-only, bounded by count, size, aggregate size, and path depth, strictly validated, compiled in memory, and discarded on page close or reload. Templates cannot execute code or load assets.
...
- Check templates for oversized input, remote/executable content, duplicate resolution, and PDFs for accidental unencrypted values or reuse of the vault password.
- Verify the production build emits the allowlisted portable folder, fixed local assets, and raw templates with no network dependency.
